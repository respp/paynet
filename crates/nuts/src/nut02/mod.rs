...
pub struct KeysetResponse<U: Unit> {
    /// set of public key ids that the mint generates
    pub keysets: Vec<KeySetInfo<U>>,
}

/// Mint Public Keys [NUT-01]
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct KeysResponse<U: Unit> {
    /// Keysets
    pub keysets: Vec<KeySet<U>>,
}

/// Keyset
#[derive(Debug, Clone, PartialEq, Eq, Deserialize, Serialize)]
pub struct KeySet<U: Unit> {
    /// Keyset [`Id`]
    pub id: KeysetId,
    /// Keyset [`CurrencyUnit`]
    pub unit: U,
...
impl<U: Unit> KeySet<U> {
    /// Verify the keyset is matches keys
    pub fn verify_id(&self) -> Result<(), Error> {
        let keys_id: KeysetId = (&self.keys).into();
...
impl<U: Unit> From<MintKeySet<U>> for KeySet<U> {
    fn from(keyset: MintKeySet<U>) -> Self {
        Self {
            id: keyset.id,
            unit: keyset.unit,
            keys: SetPubKeys::from(keyset.keys),
        }
    }
}
...
impl From<&SetKeyPairs> for KeysetId {
    fn from(keys: &SetKeyPairs) -> Self {
        let iter = keys.values().map(|keypair| keypair.public_key);
        KeysetId::from_iter(iter)
    }
}
...
mod test {
    ...
    use crate::{
        nut02::{KeysResponse, KeysetId},
        traits::test_types::TestUnit,
    };

    use super::{KeySetInfo, KeysetResponse, SetPubKeys};
    ...
    fn test_deserialization_and_id_generation() {
        let _id = KeysetId::from_str("009a1f293253e41e").unwrap();

        let keys: SetPubKeys = serde_json::from_str(SHORT_KEYSET).unwrap();

        let id: KeysetId = (&keys).into();

        assert_eq!(id, KeysetId::from_str(SHORT_KEYSET_ID).unwrap());

        let keys: SetPubKeys = serde_json::from_str(KEYSET).unwrap();

        let id: KeysetId = (&keys).into();

        assert_eq!(id, KeysetId::from_str(KEYSET_ID).unwrap());
    }
    ...
    fn test_id_from_invalid_byte_length() {
        ...
        let result = KeysetId::from_bytes(&three_bytes);
    ...
    fn test_keyset_bytes() {
        ...
        let id_from_bytes = KeysetId::from_bytes(&id_bytes).unwrap();
    ...
    fn test_id_deserialization() {
        let id_from_short_str = KeysetId::from_str("00123");
        assert!(matches!(id_from_short_str, Err(Error::Length)));

        let id_from_non_hex_str = KeysetId::from_str(&SHORT_KEYSET_ID.replace('a', "x"));
        assert!(matches!(id_from_non_hex_str, Err(Error::HexError(_))));

        let id_invalid_version = KeysetId::from_str(&SHORT_KEYSET_ID.replace("00", "99"));
        assert!(matches!(id_invalid_version, Err(Error::UnknownVersion)));

        let id_from_uppercase = KeysetId::from_str(&SHORT_KEYSET_ID.to_uppercase());
